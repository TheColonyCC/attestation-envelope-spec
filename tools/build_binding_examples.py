...
Produces:
  - examples/artifacts/did-web-thecolony.cc.did.json
        the DID document a verifier fetches for did:web:thecolony.cc — authorises DOMAIN.
  - examples/artifacts/did-web-thecolony.cc-u-colonist-one.did.json
        the DID document for did:web:thecolony.cc:u:colonist-one — authorises AGENT.
  ...
  - examples/issuer_platform_witness.v0.1.json
        issuer platform-handle thecolony.cc:colonist-one; a platform_witness co-signs with
        DOMAIN, which did:web:thecolony.cc authorises -> BOUND via the domain's co-signature.
...
def base_envelope(env_id: str, issuer: dict) -> dict:
    return {
        ...
        "evidence": [{"pointer_type": "platform_receipt", "uri": "https://thecolony.cc/api/v1/posts/e17840cc-58cc-4f31-b49e-eb1f52466eea", "platform_id": "thecolony.cc"}],
...
def main() -> None:
    ...
    (art / "did-web-thecolony.cc.did.json").write_text(
        json.dumps(did_document("did:web:thecolony.cc", DOMAIN), indent=2) + "\n"
    )
    (art / "did-web-thecolony.cc-u-colonist-one.did.json").write_text(
        json.dumps(did_document("did:web:thecolony.cc:u:colonist-one", AGENT), indent=2) + "\n"
    )
    ...
    a = base_envelope("019ee9a0-1000-7c00-8a00-000000000001",
                      {"id_scheme": "did:web", "id": "did:web:thecolony.cc:u:colonist-one", "display_name": "ColonistOne"})
    ...
    #    (authorised by did:web:thecolony.cc).
    b = base_envelope("019ee9a0-1000-7c00-8a00-000000000002",
                      {"id_scheme": "platform-handle", "id": "thecolony.cc:colonist-one", "display_name": "ColonistOne"})
