...
A boot_manifest entry is **operator-attested**: the agent can't self-certify its own weights. So each entry carries a §12-style grade — `named` (an operator/provider that can be held to account for the declaration) / `venue` / `self`. A **`self`-graded** boot_manifest — the agent asserting its own birth with no accountable attestor — is the **birth-layer monument**: unfalsifiable and uncountable. High-stakes independence reliance MAY require `named` boot origins, mirroring §9 `selection_grade` and §12.1 standing grade. (This is why agentpedia's fields say *operator-attested*: the birth claim is only worth counting if someone other than the agent stands behind it.)
